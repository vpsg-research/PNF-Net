...
class PolypObjDataset(data.Dataset):
    def __init__(self, real_image_root, fake_image_root, gt_root, edge_root, trainsize, type_='train'):
        ...
        self.real_images = [os.path.join(real_image_root, f) for f in os.listdir(real_image_root) if f.endswith('.jpg') or f.endswith('.tif') or f.endswith('.bmp') or f.endswith('.png')]
        self.fake_images = [os.path.join(fake_image_root, f) for f in os.listdir(fake_image_root) if f.endswith('.jpg') or f.endswith('.tif') or f.endswith('.bmp') or f.endswith('.png')]
        ...
        self.real_images = sorted(self.real_images)
        self.fake_images = sorted(self.fake_images)
    ...
    def __getitem__(self, index):
        ...
        lab = self.labs[index]  # 获取标签（0 或 1）
        ...
        return image, gt, eg, lab  # 返回图像、真实掩码、边缘掩码和标签

    def filter_files(self):
        print(len(self.fake_images))
        print(len(self.real_images))
        print(len(self.gts))
        assert len(self.fake_images) == len(self.gts) and len(self.gts) == len(self.egs)
        images = []
        gts = []
        labs = []
        egs = []
        for real_img_path in self.real_images:
            
            img = Image.open(real_img_path)
            real_gt = np.zeros((img.size[1], img.size[0], 3), dtype=np.uint8)  # 注意：img.size是(width, height)
            real_eg = np.zeros((img.size[1], img.size[0], 3), dtype=np.uint8)  # 注意：img.size是(width, height)
            gt_path= Image.fromarray(real_gt)  # 将numpy数组转换为PIL图像对象
            eg_path= Image.fromarray(real_eg)
            if img.size == gt.size:
        ...
        for img_path, gt_path, eg_path in zip(self.fake_images, self.gts, self.egs):
